...
UT_START_TEST(test1)
{
	...
	UT_ASSERT_EQUAL(648, score);
}
UT_END_TEST


void 
test2and3_commit_action_1(void *args, int *result)
{
...
void 
test2and3_commit_action_2(void *args, int *result)
{
	int my_seq_num = (int) args;
...
UT_START_TEST(test2)
{
	...
	XACT_BEGIN(xact_commit_action1)
		...
		XACT_BEGIN(xact_commit_action2)
			XACT_WAIVER { seq_num++; }
			txc_tx_register_commit_action(txd, test2and3_commit_action_1, (void *) seq_num, NULL, 0);
			XACT_WAIVER {
				if (retries++ == 0) {
					XACT_ABORT(TXC_ABORTREASON_USERRETRY);	
				}
		...
		XACT_END(xact_commit_action2)
		XACT_WAIVER { seq_num++; }
		txc_tx_register_commit_action(txd, test2and3_commit_action_2, (void *) seq_num, NULL, 0);
	XACT_END(xact_commit_action1)

	UT_ASSERT_EQUAL(3888, score);
}
UT_END_TEST
...
UT_START_TEST(test3)
{
	...
	UT_ASSERT_EQUAL(23328, score);
}
UT_END_TEST


void 
test4_commit_action_1(void *args, int *result)
{
	score *= pow(godel[0], ++seq_num);
}

void 
test4_commit_action_2(void *args, int *result)
{
	score *= pow(godel[1], ++seq_num);
}
...
UT_START_TEST(test4)
{
	...
	XACT_BEGIN(xact_commit_action1)
		XACT_WAIVER { seq_num++;	}
		XACT_BEGIN(xact_commit_action2)
			txc_tx_register_commit_action(txd, test4_commit_action_1, NULL, NULL, 1);
		XACT_END(xact_commit_action2)
		txc_tx_register_commit_action(txd, test4_commit_action_2, NULL, NULL, 0);
	XACT_END(xact_commit_action1)
...
{
	ut_suite_t *suite;
	ut_suite_create(&suite, "test_commit_action");
