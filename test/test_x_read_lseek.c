...
char *test_file = "/tmp/test_x_read.tmp";
...
UT_START_TEST(test1)
{
	...
	strncpy(str1, &test_file_initial_contents[66], 10);
	str1[10] = '\0';
	len1 = strlen(str1);
	strncpy(str2, &test_file_initial_contents[76], 10);
	str2[20] = '\0';
	len2 = strlen(str2);
	strncpy(str3, &test_file_initial_contents[126], 10);
	str3[20] = '\0';
	len3 = strlen(str3);
	...
	XACT_BEGIN(xact_1)
		...
		_XCALL(x_lseek)(fd, 40, SEEK_CUR, NULL);
		ret = _XCALL(x_read)(fd, buf3, len3, &result);
		XACT_WAIVER {
			UT_ASSERT_STR_EQUAL(str1, buf1);
			UT_ASSERT_STR_EQUAL(str2, buf2);
			UT_ASSERT_STR_EQUAL(str3, buf3);
		}
		XACT_WAIVER {
			if (test_retries++ <= 8) {
...
{
	ut_suite_t *suite;
	ut_suite_create(&suite, "test_x_read_lseek");
