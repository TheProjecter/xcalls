...
UT_START_TEST_THREAD(test1_thread)
{
	...
	for (i=0; i<32; i++) {
		...
		XACT_BEGIN(xact_1)
			...
			_XCALL(x_lseek(fd, -1024, SEEK_CUR, NULL));
			for (j=0; j<1024; j++) {
				ret = _XCALL(x_read)(fd, buf, 1, &result);
				XACT_WAIVER {
					UT_ASSERT_EQUAL(buf[0], (int) '0' + tid);
				}	
			}
...
UT_START_TEST_THREAD(test2_thread)
{
	...
	for (i=0; i<32; i++) {
		...
		XACT_BEGIN(xact_1)
			...
			_XCALL(x_lseek(test2_fd, -1024, SEEK_CUR, NULL));
			for (j=0; j<1024; j++) {
				ret = _XCALL(x_read)(test2_fd, buf, 1, &result);
				XACT_WAIVER {
					UT_ASSERT_EQUAL(buf[0], (int) '0' + tid);
				}	
			}
...
{
	ut_suite_t *suite;
	ut_suite_create(&suite, "test_x_open_write_read_lseek_multithread");
