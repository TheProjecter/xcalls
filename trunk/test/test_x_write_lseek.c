...
UT_START_TEST(test1)
{
	...
	final_contents = str_create(test_file_initial_contents);
	intermediate_contents = str_create(test_file_initial_contents);
	str_modify(&intermediate_contents, 66, "DEADBEEF");
	str_modify(&intermediate_contents, 78, "MADCOW");
	str_modify(&intermediate_contents, 86, "ZOMBIE");

	test_retries = 0;
	fd = _XCALL(x_open)(test_file, O_RDWR, S_IRUSR|S_IWUSR, NULL);
	XACT_BEGIN(xact_1)
		_XCALL(x_lseek)(fd, 66, SEEK_CUR, NULL);
		ret = _XCALL(x_write_ovr_save)(fd, "DEADBEEF", 8, &result);
		_XCALL(x_lseek)(fd, 4, SEEK_CUR, NULL);
		ret = _XCALL(x_write_ovr_save)(fd, "MADCOW", 6, &result);
		_XCALL(x_lseek)(fd, 2, SEEK_CUR, NULL);
		ret = _XCALL(x_write_ovr_save)(fd, "ZOMBIE", 6, &result);
		XACT_WAIVER {
			UT_ASSERT_EQUAL(UT_TRUE, file_equal_str(test_file, 
...
UT_START_TEST(test2)
{
	...
	UT_ASSERT_EQUAL(UT_TRUE, file_equal_str(test_file, test_file_initial_contents));
	final_contents = str_create(test_file_initial_contents);
	str_modify(&final_contents, 66, "DEADBEEF");
	str_modify(&final_contents, 78, "MADCOW");
	str_modify(&final_contents, 86, "ZOMBIE");
...
UT_START_TEST(test4)
{
	...
	str_modify(&intermediate_contents, 66, "DEADBEEF");
	str_modify(&intermediate_contents, 78, "MADCOW");
	str_modify(&intermediate_contents, strlen(final_contents), "ZOMBIE");
	...
	XACT_BEGIN(xact_1)
		...
		_XCALL(x_lseek)(fd, 4, SEEK_CUR, NULL);
		ret = _XCALL(x_write_ovr_save)(fd, "MADCOW", 6, &result);
		_XCALL(x_lseek)(fd, 0, SEEK_END, NULL);
		ret = _XCALL(x_write_ovr_save)(fd, "ZOMBIE", 6, &result);
		XACT_WAIVER {
			UT_ASSERT_EQUAL(UT_TRUE, file_equal_str(test_file, 
			                                        intermediate_contents));
...
UT_START_TEST(test5)
{
	...
	final_contents = str_create(intermediate_contents);
...
{
	ut_suite_t *suite;
	ut_suite_create(&suite, "test_x_write_lseek");
